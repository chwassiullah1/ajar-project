...
class _VehicleUpdateScreenState extends State<VehicleUpdateScreen> {
  ...
  void initState() {
    ...
    _vehicleRegistraionNumberController.text =
        widget.vehicle.registrationNumber;
    _vehicleLicenseNumberController.text = widget.vehicle.licensePlate;
    _vehicleEngineSizeController.text = widget.vehicle.engineSize.toString();
    _vehicleSeatsCapcityController.text = widget.vehicle.seats.toString();
    _vehicleMileageController.text = widget.vehicle.mileage.toString();
    _vehicleColorController.text = widget.vehicle.color;
    _vehicleDesctiptionController.text = widget.vehicle.description;
    _vehicleReasonController.text = widget.vehicle.reasonForHosting;
    _pickedDatesController.text =
        "${formatDate(widget.vehicle.startDate)} / ${formatDate(widget.vehicle.endDate)}";
    _pickedVehicleLocationController.text = widget.vehicle.fullAddress;
    _vehicleVinController.text = widget.vehicle.vinNumber;
    _vehiclePricePerDayController.text = widget.vehicle.price;
    _addressCityController.text = widget.vehicle.city;
    _driverPriceController.text = widget.vehicle.driverPrice;
    _deliveryCheckbox = widget.vehicle.deliveryAvailable;
    _deliveryChargesController.text = widget.vehicle.deliveryPrice.toString();
    selectedDriverOption = widget.vehicle.driverAvailability;
    _selectedFuelType.text = widget.vehicle.fuelType;
    _selectedTransmissionType.text = widget.vehicle.transmissionType;
    _selectedVehicleType.text = widget.vehicle.vehicleType;
    startDate = widget.vehicle.startDate;
    endDate = widget.vehicle.endDate;
    latitude = double.parse(widget.vehicle.latitude);
    longitude = double.parse(widget.vehicle.longitude);
    _isAc = widget.vehicle.isAc;
    _isBluetooth = widget.vehicle.isBluetooth;
    _isCharger = widget.vehicle.isCharger;
    _isGps = widget.vehicle.isGps;
    _isPushButtonStart = widget.vehicle.isPushButtonStart;
    _isSunroof = widget.vehicle.isSunroof;
    _isUsb = widget.vehicle.isUsb;
    // Other initializations
    // Load existing vehicle images
    // Load existing images
    if (widget.vehicle.pictures != null) {
      for (int i = 0; i < widget.vehicle.pictures!.length; i++) {
        selectedImages[i] = widget.vehicle.pictures![i];
      }
    }
  }
  ...
  // Existing image URLs (fetched from the API)
  //List<String?> existingImages = List<String?>.filled(6, null);

  // New images picked by the user (using XFile)
  //List<XFile?> selectedImages = List<XFile?>.filled(6, null);
  List<dynamic> selectedImages = List<dynamic>.filled(6, null); // Mixed types
    ...
    if (pickedImage != null) {
      setState(() {
        selectedImages[imageIndex - 1] =
            pickedImage; // Replace image at the specified index
      });
    }
  }
  ...
  //   if (pickedImage != null) {
  //     setState(() {
  //       // Store the new image in selectedImages and clear existingImages at that index
  //       selectedImages[imageIndex - 1] = pickedImage;
  //       existingImages[imageIndex - 1] = null;
  //     });
  //   }
  // }
                    ...
                    child: GestureDetector(
                      onTap: () {
                        openModelSelectionDialog(
                            context, _modelController, _makeController.text);
                      },
                      child: AbsorbPointer(
                        child: CustomTextFormField(
                          label: 'Model',
          ...
          children: [
            ...
            CustomGradientButton(
              ...
              onPressed: () async {
                ...
                // Create a dummy vehicle object with updated data
                Vehicle updatedVehicleData = Vehicle(
                  ...
                  pictures: null,
                  registrationNumber: _vehicleRegistraionNumberController.text,
                  licensePlate: _vehicleLicenseNumberController.text,
                  description: _vehicleDesctiptionController.text,
                  driverAvailability: selectedDriverOption,
                  price: _vehiclePricePerDayController.text,
                  ...
                  isUsb: _isUsb,
                  driverPrice: _driverPriceController.text,
                  deliveryAvailable: _deliveryCheckbox,
                  deliveryPrice: _deliveryChargesController.text,
                );

                // Call the provider method to update the vehicle
                int statusCode = await vehicleProvider.updateVehicle(
                    updatedVehicleData, widget.vehicle.id!);

                if (statusCode == 200) {
                  // Vehicle updated successfully
                  final currentVehicleID = vehicleProvider.vehicle!.id!;

                  // Filter only the new images selected by the user
                  // List<XFile> imagesToUpload = selectedImages
                  ...
                  // Upload pictures only if there are new images selected
                  if (imagesToUpload.isNotEmpty) {
                    int uploadStatusCode =
                        await vehicleProvider.uploadVehiclePictures(
                            currentVehicleID, imagesToUpload);
                    ...
                    if (uploadStatusCode == 200) {
                      ...
                      showCustomSnackBar(context,
                          'Failed to upload updated pictures.', Colors.red);
                    }
                  } else {
                    // No new images selected; skip uploading images
                    showCustomSnackBar(context,
                        "Vehicle details updated successfully!", Colors.green);
                  }

                  // Close all the navigation pop-ups
                  Navigator.pop(context);
                  Navigator.pop(context);
                  Navigator.pop(context);
                } else if (statusCode == 400) {
                  showCustomSnackBar(context,
                      'Error! Fill all details carefully!', Colors.red);
                  ...
                  showCustomSnackBar(
                      context, 'Failed to update vehicle.', Colors.red);
                }
              },
            ),
          ],
...
// Step 2: Modify _buildImageContainer to handle both XFile? and String?
  Widget _buildImageContainer(
      BuildContext context, dynamic imageFile, int index) {
    Brightness brightness = Theme.of(context).brightness;
    bool isDarkMode = brightness == Brightness.dark;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: imageFile == null
            ...
            : CircleAvatar(
                backgroundImage:
                    imageFile is String && imageFile.startsWith('http')
                        ? NetworkImage(imageFile) // Use NetworkImage for URL
                        : imageFile is XFile
                            ? FileImage(
                                File(imageFile.path)) // Use FileImage for XFile
                            : null,
                radius: 50,
              ),
      ),
